...
export default function TextBox({name, placeholder, onchange, getValue, type, success , value }) {
  ...
  return (
    <div className='relative w- max'>
      {type === "password" && <BiSolidHide onClick={() => setInputType(inputType === "password" ? "text" : "password")} className=" w-6 h-6 sm:w-8 sm:h-8 absolute top-3 sm:top-4 right-2 cursor-pointer" color='#9B9B9A' />}
